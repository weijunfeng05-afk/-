...
import {jobOutput} from './ai/schema';
export async function processJob(id:string,owner:string){
 ...
 if(!job)return;
 try{
  const result=await callAI('job',id,owner,JOB_PROMPT,{name:job.name,jd:job.jd,department:job.department,location:job.location,level:job.level,notes:job.notes},x=>jobOutput.parse(x));
  await d.batch([
